...
This work focuses solely on predicting diffuse garment textures. 
To enable high-quality rendering, future work should extend the model to predict normal and roughness maps, 
potentially by decoding latents using DressCode's modality-specific VAE decoders~\cite{dresscode}. 
We find that strong performance can be achieved with limited training data, suggesting that dataset diversity may be a limiting 
factor—an avenue worth further investigation. Another key direction is evaluating generalization to real-world images. 
...
\section{Conclusion}
We proposed a method for predicting diffuse garment texture maps from a 
single (masked) image. By finetuning a pre-trained diffusion model on a synthetic dataset of 
27,000 PBR textures, we demonstrated high-quality results. 
Nevertheless, this represents a first step toward fully automated garment digitalization. 
Future work should aim to extend the approach to predict normal and roughness maps and 
assess its generalization to real-world images.
