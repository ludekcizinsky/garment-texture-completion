...
class StableDiffusionInstructPix2PixPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def prepare_image_latents(
        ...
        if mask is not None:
            ...
            mask = torch.from_numpy(mask)[None].to(device=device, dtype=dtype)
            # print(f"mask.shape: {mask.shape}")
            latent_shape = image_latents.shape[2:]  # Get latent spatial dimensions
            # Resize mask (assume it has shape [B, 1, H_mask, W_mask])
            mask_embeds = F.interpolate(mask.unsqueeze(1).float(), size=latent_shape, mode="nearest")
            # mask_embeds = mask_resized.bool()  # Back to original dtype and shape
            # print(f"mask_embeds.shape: {mask_embeds.shape}")
            image_latents = torch.cat([image_latents, mask_embeds], dim=1)
